...
<Callout type="warning">
  **`BASE_PATH` is consumed at build time, not at run time.** Next.js bakes
  ...
  the root. To deploy under a subpath you have to build your own frontend image
  with `BASE_PATH` set in the build environment. Every other variable on this
  page is read at run time and works from `.env` as expected.
</Callout>
