...
and [Configuration → Plugins](/self-hosting/configuration#plugins) (incl.
...
Any tool that fetches a URL the _model_ picked — the `fetchUrl` tool — runs the
target through a core guard first. It resolves the hostname and refuses the
fetch if **any**
...
backend type; omit it and that backend can't be selected at all. See
[Extending → Sandbox backends](/extending/sandbox-backends) and
