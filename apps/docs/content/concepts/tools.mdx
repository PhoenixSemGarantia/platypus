...
- **contributed by a plugin** — a plugin declares tool sets in its manifest and
  ...
  with Platypus and **third-party plugins** installed as npm packages. Which
  ones a deployment has depends on which plugins its Operator enabled; the
  Agent form's **Tools** card is the live list. See [Extending → Tool
  sets](/extending/tool-sets) for how one is contributed.
