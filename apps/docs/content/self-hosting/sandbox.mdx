...
<Callout type="warning">
  **The overlay replaces `PLATYPUS_PLUGINS`; it does not merge.** It sets the
  variable under `services.backend.environment`, which outranks the base
  `env_file: .env` — so whatever list you put in `.env` is discarded. The
  shipped overlay reads `@platypus/web-fetch,@platypus/docker`; add
  `@platypus/ssh` (or a third-party plugin) to `.env` and compose the overlay,
  and it is silently dropped with no error. Edit the list **inside
  `compose.sandbox.yaml`** to your deployment's complete gate-able set, then
  confirm what loaded in **Organization settings → Plugins**.
</Callout>
...
"Recreate now" button or "pending" badge in the app to tell you so. The Sandbox
screen offers **Save** and **Delete**, nothing else.
