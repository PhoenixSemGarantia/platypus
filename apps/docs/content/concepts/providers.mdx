...
Not every model can read every file. Which types a model takes **natively** is
declared per model on the Provider, and what happens to an attachment follows
from that:
...
- **It's a PDF or DOCX the model can't take natively** — the text is extracted
  ...
  attachments usable with local and proxied models.
...
<Callout type="info">
  Extracted text is **lossy**: tables, layout, and embedded images don't survive,
  and a long document is truncated with a note saying so. If fidelity matters,
  use a model that reads the format natively. There's no OCR, so a scanned
  (image-only) PDF has no text to extract and is refused.
</Callout>
...
Chat warns you when the model you've selected can't read an attachment natively,
