...
<Callout type="info">
  ...
  out so its network egress can be denied in isolation),
  `@platypus/tools-platform` (the Platypus-domain sets — kanban, dashboards,
  triggers, memory, notifications, agent discovery/management, skill
  management), and `@platypus/docker` (the Sandbox backend). **Third-party
